        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-8">
          {solutions.map((s) => (
            <Link 
              key={s.id} 
              href={s.href}
              className="glass-card p-10 rounded-3xl group border-slate-100/50"
            >
              <div className="flex flex-col h-full">
                <h3 className="text-2xl font-bold mb-4 group-hover:text-indigo-600 transition-colors tracking-tight">{s.title}</h3>
                <p className="text-slate-500 mb-8 flex-grow leading-relaxed">{s.desc}</p>
                <span className="inline-flex items-center text-xs font-black text-slate-950 gap-2 uppercase tracking-widest">
                  Learn more
